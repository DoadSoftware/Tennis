...
public class ATP_2022 extends Scene {
	...
	public Object ProcessGraphicOption(String whatToProcess, Match match, TennisService tennisService,
		...
		case "ANIMATE-IN-H2H_LT":
		case "ANIMATE-IN-H2H_FF":
		case "ANIMATE-OUT":
			switch (whatToProcess.toUpperCase()) {
			...
			case "ANIMATE-IN-SCOREBUG":
				...
				break;
			case "ANIMATE-IN-H2H_LT":
				AnimateInGraphics(print_writer, "LT_H2H");
				which_graphics_onscreen = "LT_H2H";
				break;
			case "ANIMATE-IN-H2H_FF":
				AnimateInGraphics(print_writer, "FF_H2H");
				which_graphics_onscreen = "FF_H2H";
				break;
			case "CLEAR-ALL":
...
//								}
		...
		if (match == null) {
			...
			if (match.getMatchType().toUpperCase().equalsIgnoreCase("SINGLES")) {
				...
				print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tCountry1" + " SET "
						+ plyr.get(match.getHomeFirstPlayerId() - 1).getNationality() + "\0");
				print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tRank1" + " SET "
						+ plyr.get(match.getHomeFirstPlayerId() - 1).getRank() + "\0");

				print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tFirstName1"
						+ " SET " + match.getHomeFirstPlayer().getFirstname().toUpperCase() + "\0");
				if (match.getHomeFirstPlayer().getSurname() == null) {
					print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tLastName1"
							+ " SET " + "" + "\0");
				} else {
					print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tLastName1"
							+ " SET " + match.getHomeFirstPlayer().getSurname().toUpperCase() + "\0");
				}

				print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tCountry2" + " SET "
						+ plyr.get(match.getAwayFirstPlayerId() - 1).getNationality() + "\0");
				
				print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tRank1" + " SET "
						+ plyr.get(match.getAwayFirstPlayerId() - 1).getRank() + "\0");

				print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tFirstName2"
						+ " SET " + match.getAwayFirstPlayer().getFirstname().toUpperCase() + "\0");
				if (match.getAwayFirstPlayer().getSurname() == null) {
					print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tLastName2"
							+ " SET " + "" + "\0");
				} else {
					print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tLastName2"
							+ " SET " + match.getAwayFirstPlayer().getSurname().toUpperCase() + "\0");
				}
				
			} else if (match.getMatchType().toUpperCase().equalsIgnoreCase("DOUBLES")) {
				print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tFirstName1"
						+ " SET " + match.getHomeFirstPlayer().getTicker_name().toUpperCase() + "\0");
				print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tLastName1" + " SET "
						+ match.getHomeSecondPlayer().getTicker_name().toUpperCase() + "\0");

				print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tFirstName2"
						+ " SET " + match.getAwayFirstPlayer().getTicker_name().toUpperCase() + "\0");
				print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "tLastName2" + " SET "
						+ match.getAwaySecondPlayer().getTicker_name().toUpperCase() + "\0");
			}
		}
	...
	public void populateFFH2H(PrintWriter print_writer, String viz_sence_path, Match match,
